<?php
    ...
    if ($filtro == 1) {
        ...
        $sql = "SELECT iddocumentacao FROM documentacao WHERE descricao = '".$_POST['descricao']."'";
        $res = mysql_query($sql);
        $ret = mysql_num_rows($res);

            if ($ret != 0) {
                echo'Item da documenta&ccedil;&atilde;o j&aacute; cadastrado.'; exit;
            }

        unset($sql,$res,$ret);

        //insere no banco
        $sql = "INSERT INTO documentacao (descricao,monitor) VALUES ('".$_POST['descricao']."','O')";
        $res = mysql_query($sql);
